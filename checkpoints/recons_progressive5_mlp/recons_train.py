...
model = __import__('models.recons', fromlist=[''])
model = model.ReconsModel(opt)
...
for epoch in range(start_epoch, opt.niter + opt.niter_decay + 1):
    ...
    for i, (data_3D, data_2D) in enumerate(train_dataloader):
        ...
        losses, generated = model(*input)

        loss_dict = dict(zip(model.module.loss_names, losses))
        loss = 0.0001*loss_dict.get('Recons',0) + 0.1*loss_dict.get('Proj',0)
